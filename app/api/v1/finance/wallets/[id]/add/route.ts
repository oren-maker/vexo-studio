...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return ok(await prisma.$transaction(async (tx) => {
      const updated = await tx.creditWallet.update({ where: { id: params.id }, data: { availableCredits: { increment: body.amount }, totalCreditsAdded: { increment: body.amount } } });
      await tx.creditTransaction.create({ data: { walletId: updated.id, transactionType: "ADD", amount: body.amount, unitType: body.unitType, sourceType: "MANUAL", description: body.description, createdByUserId: ctx.user.id, referenceId: body.referenceId } });
      return updated;
    }));
