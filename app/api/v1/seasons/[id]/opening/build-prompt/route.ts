...
${body.model.startsWith("sora") ? `IMPORTANT — target is OpenAI Sora. Sora has rejected our prior openings. Follow these HARD constraints or the prompt will be blocked again:
(1) USE ONLY POSITIVE PHRASING. Never write "no people", "no one", "without anyone", "empty of", "lacking", "not a soul", "uninhabited" — instead just describe the present objects without mentioning absent people at all.
(2) NEVER imply physics defiance. Forbidden phrases: "against gravity", "defying gravity", "defies physics", "impossibly", "supernaturally", "magically". If something rises, write "drifts upward", "rises slowly", "lifts gently", "floats" — purely observational.
(3) NEVER label something as strange or otherworldly. Forbidden words: dreamlike, surreal, uncanny, eerie, strange, mysterious, anomaly, glitch, lag, delayed, anomalous, otherworldly, odd, weird, off, unsettling, ominous, tense.
(4) FORBIDDEN topical words (Sora moderation): paranoid, drugs, tattoos, blood, violence, sexual, surveillance, suspicious, threatening, dark, shadow-heavy, psychological, thriller, noir, crime, espionage, weapon, gun, knife, soldier, military, combat, kill, death, murder.
(5) PREFER: serene, graceful, luminous, tranquil, golden, gentle, slow, quietly, softly, calm, warm.
(6) For visual mysteries — describe what is SEEN, not how it feels. Example: instead of "the mirror reflects with a dreamlike half-second delay", write "the mirror's reflection follows a beat after the source moves".
(7) Sora can lock ONE face via the starting image but cannot multi-reference. For every OTHER character include a rich physical description in the prompt itself (hair color and length, skin tone, build, signature wardrobe).` : ""}
