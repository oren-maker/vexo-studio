...
export default function CalendarPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">{he ? "לוח שנת תוכן" : "Content Calendar"}</h1>
      <Card title={he ? "מתוזמנים" : "Scheduled"} subtitle={`${items.length} ${he ? "פריטים" : "entries"}`}>
        <div className="flex justify-end mb-4">
          <button onClick={() => setCreating(true)} className="px-3 py-1.5 rounded-lg bg-accent text-white text-sm font-semibold">+ {he ? "תזמן" : "Schedule"}</button>
        </div>
        {creating && (
          <form onSubmit={add} className="bg-bg-main rounded-lg p-4 mb-4 grid grid-cols-3 gap-2">
            <input name="title" required placeholder={he ? "כותרת" : "Title"} className="px-3 py-2 rounded-lg border border-bg-main bg-white" />
            ...
            <button className="col-span-3 px-4 py-2 rounded-lg bg-accent text-white text-sm">{he ? "תזמן" : "Schedule"}</button>
          </form>
        )}
        {items.length === 0 ? (
          <div className="text-center py-8 text-text-muted"><div className="text-3xl mb-2">📅</div><div>{he ? "אין פריטים מתוזמנים עדיין." : "Nothing scheduled yet."}</div></div>
        ) : (
          <ul className="space-y-2">
            {items.map((e) => (
              <li key={e.id} className="bg-bg-main rounded-lg p-3 flex justify-between items-center">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-bg-card font-bold">{he ? (STATUS_HE[e.status] ?? e.status) : e.status}</span>
              </li>
            ))}
          </ul>
        )}
